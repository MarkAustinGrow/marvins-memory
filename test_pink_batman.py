...
async def test_pink_batman_tweet():
    """Test processing the Pink Batman tweet"""
    
    # Create a mock tweet object
    tweet = {
        "id": 10599,
        "tweet_id": "1922949472088203571",  # Using a unique ID
        "tweet_text": "RT @LudovicCreator: My Pink Batman",
        ...
        "vibe_tags": "art,creative,batman",
        "created_at": "2025-05-15 09:52:19.903"
    }
    
    logger.info("Starting Pink Batman tweet test")
    
    try:
        # First, evaluate if the tweet is worth researching
        ...
        if evaluation.get("is_worth_researching", False):
            logger.info("Tweet deemed worth researching!")
            ...
            if result.get("status") == "error":
                ...
                logger.info(f"Created {len(memory_ids)} memories")
